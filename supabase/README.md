...
| 12 | `0012_redeem_invite_variable_conflict.sql` | `#variable_conflict use_column` in redeem_invite. Postgres 15+'s stricter plpgsql resolver flagged `on conflict (household_id, user_id)` as ambiguous against the OUT param of the same name — every invite redemption failed with 42702. |
| 13 | `0013_household_cascade.sql`     | Rewrites every `household_id` FK on the 15 feature tables to `ON DELETE CASCADE`. Without it a household delete silently no-op'd (23503) as soon as it had any pet / doc / event attached. |
| 14 | `0014_realtime_household_members.sql` | Adds `household_members` to `supabase_realtime`. Without it the owner had to restart the app to see a joined member — no postgres_changes event fired for M0's members list. |
